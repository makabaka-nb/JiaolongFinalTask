...
class Motor {
public:
    Motor(uint8_t escid, float ratio, ControlMethod control_method,uint8_t M3508_flag,uint8_t GM6020_flag,PID spid,PID ppid);
    ...
    float GetLimitedMinAngle(){return limited_min_angle_;}
    float GetLimitedMaxAngle(){return limited_max_angle_;}
    void SetAngleLimit(float min_angle,float max_angle) {
        limited_min_angle_ = min_angle;
        limited_max_angle_ = max_angle;
    }
    float GetAngle(){return angle_;}
    static void SendControlData(CAN_HandleTypeDef *hcan);
private:
    // 电机属性
    const uint8_t escid_; // 电调号（1-8）
    const float ratio_; // 减速比
    const float torque_constant_ = 0.3f; // 力矩常数 Nm/A 已经包括减速比
    const uint8_t M3508_flag_;
    const uint8_t GM6020_flag_;
    // 电机状态
    ...
    static constexpr uint16_t M3508_TX_ID_1_4 = 0x200; // 1-4号电调发送标识符
    static constexpr uint16_t M3508_TX_ID_5_8 = 0x1FF; // 5-8号电调发送标识符
    static constexpr uint16_t GM6020_TX_ID_1_4 = 0x1FE; // 1-4号电调发送标识符
    static constexpr uint16_t GM6020_TX_ID_5_8 = 0x2FE; // 5-8号电调发送标识符
    // 电机限制参数
    float MAX_CURRENT = 3.0f; // 最大电流限制
    float MAX_SPEED = 200.0f; // 最大转速限制
    ...
    float limited_min_angle_=-90.0f;
    float limited_max_angle_=90.0f;
